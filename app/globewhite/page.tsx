...
// Global styles for ripple effect on click and pulse animation - LIGHT MODE
if (typeof document !== 'undefined') {
  ...
  style.textContent = `
    ...
    .ripple-effect-light {
      ...
      pointer-events: none;
      animation: ripple-expand-light 0.6s cubic-bezier(0.25, 0.1, 0.25, 1) forwards;
      z-index: 1000;
    }
  `
  if (!document.getElementById('globey-ripple-animation-light')) {
    style.id = 'globey-ripple-animation-light'
...
// Helper function to get source icon based on source name - LIGHT MODE
...
export default function Home() {
  ...
  // Toggle map style handler - LIGHT MODE ADJUSTED
  const toggleMapStyle = useCallback(() => {
    ...
    map.current.once('style.load', () => {
      ...
      // Re-add base GPU-rendered dots layer - DARK DOTS FOR LIGHT MODE
      ...
      // Re-add glow layers - DARK GLOW FOR LIGHT MODE
      ...
      // Re-add equator ring layers - DARK FOR LIGHT MODE
      ...
      // Re-add country border sources and layers separately - DARK FOR LIGHT MODE
    ...
    const initMap = async () => {
      try {
        ...
        map.current = new mapboxgl.Map({
          ...
          style: 'mapbox://styles/mapbox/streets-v12', // LIGHT MODE STYLE
          center: [randomLng, 30],
          zoom: 0.5,
        ...
        map.current.on('load', async () => {
          ...
          // Set globe atmosphere - PURE WHITE
            map.current.setFog({
              'horizon-blend': 0.1,
            color: '#ffffff',
              'high-color': '#ffffff',
          ...
          // Outer glow layer for the ring - DARK FOR LIGHT MODE
          map.current.addLayer({
            id: 'equator-ring-glow',
            type: 'line',
            source: 'equator-ring',
          ...
          // Main ring line - DARK FOR LIGHT MODE
          map.current.addLayer({
            id: 'equator-ring-line',
            type: 'line',
            source: 'equator-ring',
          ...
          // Base GPU-rendered dots layer for smooth globe tracking - DARK DOTS FOR LIGHT MODE
          map.current.addLayer({
            id: 'stories-dots',
            type: 'circle',
            source: 'stories',
          ...
          // Create multiple glow layers for enhanced visual effect - DARK GLOW FOR LIGHT MODE
          ...
          // Then add country border highlight layers - DARK FOR LIGHT MODE
          map.current.addLayer({
            id: 'country-border-glow',
            type: 'line',
            source: 'country-border-glow',
  ...
  return (
    ...
    <div className="relative w-full h-screen bg-white overflow-hidden">
      ...
      {/* Top Right Controls - Map Style Toggle - LIGHT MODE */}
      {globeData && !isLoading && !selectedLocation && (
        <div className="absolute top-4 right-4 z-10">
          <button
            onClick={toggleMapStyle}
            className="p-2.5 bg-white/90 backdrop-blur border border-gray-300 rounded-lg text-gray-800 hover:bg-gray-100 hover:border-gray-400 transition-all shadow-md"
...
function MapMarker({
  ...
  return (
    <div className="relative" style={{ zIndex: showPanel ? 1 : 100 }}>
      <div
        className="bg-gray-900 rounded-full transition-all duration-100 ease-out cursor-pointer"
        style={{
          width: `${baseSize}px`,
          height: `${baseSize}px`,
          ...
          boxShadow: '0 1px 3px rgba(0,0,0,0.3), inset 0 0 2px rgba(0,0,0,0.3)',
          position: 'relative',
      ...
      >
        {ripples.map(rippleId => (
          <div key={rippleId} className="ripple-effect-light" />
        ))}
      </div>

      {/* Hover/Click Panel with slide-in fade animation - LIGHT MODE */}
      {showPanel && (
        <div
        ...
        >
          <div
            className={`bg-white/95 backdrop-blur-xl border border-gray-300 rounded-xl text-gray-900 shadow-2xl transition-all ${
              location.storyCount > 1 ? 'cursor-pointer hover:border-gray-400 hover:bg-white' : ''
            }`}
            ...
            {!isExpanded ? (
              ...
              <div className="p-3 flex items-start gap-3">
                ...
                {primaryStory?.mediaUrl ? (
                  ...
                  <div className="w-24 h-16 flex-shrink-0 rounded-lg overflow-hidden bg-gray-200 flex items-center justify-center">
                    <img
                      src="/icons/newspaper.png"
                      alt="location"
                      className="w-6 h-6"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {(primaryStory?.sourceName || primaryStory?.created) && (
                    <p className="text-[9px] text-gray-600 mt-1 flex items-center gap-1 flex-wrap">
              ...
              <div className="p-3">
                ...
                <div className="flex items-start justify-between mb-3 pb-2 border-b border-gray-300">
                  <div>
                    <h3 className="text-sm font-bold text-gray-900">{location.name}</h3>
                    <p className="text-[10px] text-gray-600">{location.storyCount} {location.storyCount === 1 ? 'story' : 'stories'}</p>
                  </div>
                  <button
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto pr-2" style={{
                  ...
                  {location.stories.map((story, index) => (
                    <div key={story.id || index} className="flex items-start gap-2 pb-3 border-b border-gray-200 last:border-0">
                      {/* Story thumbnail */}
                      {story.mediaUrl ? (
